...
function createThemeColors(colorSpace) {
  return fromEntries(colorNames.map((color) => [color, `${colorSpace}(var(--color-${color}) / <alpha-value>)`]));
}
...
function flattenThemeColors(themeColors, keys2 = [], memo) {
  return entries(themeColors).reduce((memo2, [key, value]) => {
    if (typeof value === "string") {
      memo2[(key === "DEFAULT" ? keys2 : [...keys2, key]).join("-")] = value;
    } else if (value) {
      flattenThemeColors(value, [...keys2, key], memo2);
    }
    return memo2;
  }, memo ?? {});
}
function processThemeColors(themeColors, colorSpace) {
  var _a, _b;
  const colors2 = flattenThemeColors(themeColors);
...
function colorVariableValue(color, colorSpace, decimals = 4) {
  try {
    if (colorSpace === "rgb") {
      const computedColor = typeof color === "string" ? rgb4(color) : color;
      if (computedColor) {
        const { r: r2, g, b } = computedColor;
        return `${round2(r2 * 255, decimals)} ${round2(g * 255, decimals)} ${round2(b * 255, decimals)}`;
      }
    } else if (colorSpace === "hsl") {
      const computedColor = typeof color === "string" ? hsl2(clampRgb(color)) : color;
      if (computedColor) {
        const { h, s, l } = computedColor;
        return `${round2(h ?? 0, decimals)} ${round2(s * 100, decimals)}% ${round2(l * 100, decimals)}%`;
      }
    } else if (colorSpace === "oklch") {
      const computedColor = typeof color === "string" ? oklch(clampRgb(color)) : color;
      if (computedColor) {
        const { l, c: c2, h } = computedColor;
        return `${round2(l, decimals)} ${round2(c2, decimals)} ${round2(h ?? 0, decimals)}`;
      }
    }
  } catch (e4) {
  }
}
function themeStylesString(theme, colorSpace) {
...
var getDefaultConfig = () => {
  ...
  const contrast2 = fromTheme("contrast");
...
var clsMerge = (...inputs) => mergeWith_default({}, ...inputs.filter(Boolean), (a, b) => twMerge2(a, b));
...
//# sourceMappingURL=chunk-7SSTBCMY.js.map
